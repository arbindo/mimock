...
const createMock = async (requestData, token) => {
    return await post(`/mocks`, requestData, token, 'multipart/form-data; boundary=----WebKitFormBoundaryCRo3Ba2zzMcT9X03');
}

const updateMock = async (id, requestData, token) => {
    return await put(`/mocks/${id}`, requestData, token, 'multipart/form-data; boundary=----WebKitFormBoundaryCRo3Ba2zzMcT9X03');
}
